...
///
/// The line reads `* [PATH:ROW:COL] CLIPBOARD`, where the leading `*` marks a
/// buffer with unsaved edits and the row and column are 1-based. The whole row
/// is padded so the reverse-video style reaches the right edge.
///
